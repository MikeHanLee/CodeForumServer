...
//查询数据库中指定分类的博客信息
$sql=mysqli_query($conn,"SELECT `date`,`blog`.`phone`,`title`,`content`,`url`,`classification`,`commentName`,`name`,`icon` FROM `blog` INNER JOIN `user` ON `blog`.`phone` = `user`.`phone` WHERE `classification`='$getClassification' ORDER BY `date` DESC");
$result=array();
